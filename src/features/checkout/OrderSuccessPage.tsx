...
import { MinusIcon, PlusIcon, TrashIcon, WalletIcon } from "@heroicons/react/24/outline";
import { selectUser } from "../auth/authSlice";
import { addDays, format } from "date-fns";
import { atransactionOfAUser, getAUserTransaction, selectCheckout } from "./checkoutSlice";
...
console.log('data ', data)
  ...
  const calculateTotal = () => {
   const total = aUserTransactions && aUserTransactions.order && aUserTransactions.order.productDetails && aUserTransactions.order.productDetails.reduce((acc: any, cur: any ) =>     acc + cur.price * cur.quantity , 0);
    return total;
  }
  ...
  <div className="w-full max-w-7xl px-4 md:px-5 lg-6 mx-auto">
    ...
    <p className="font-normal text-lg leading-8 text-gray-500 mb-11">
      Your order and payment has been confirmed and be delivered in the next 3 days .
    </p>
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-8 py-6 border-y border-gray-100 mb-6">
      <div className="box group">
        ...
        <h6 className="font-semibold font-manrope text-2xl leading-9 text-black">
         { !Object.keys(aUserTransactions).length ? (
          ...
          <>
          {format(addDays(aUserTransactions.createdAt, 3), 'MM dd, yyyy')}
          </>
         )} 
        </h6>
      </div>
      <div className="box group">
        ...
        <h6 className="font-semibold font-manrope text-2xl leading-9 text-black">
          ...
          <>
          #{aUserTransactions && aUserTransactions.paymentDetails && aUserTransactions.paymentDetails.tx_ref} 
          </>
           )}   </h6>
      </div>
      <div className="box group">
        ...
        <div className="flex"><WalletIcon width={30} height={30} fill="brown" color="white" /><strong>FlutterWave</strong></div>
      </div>
      <div className="box group">
        <p className="font-normal text-base leading-7 text-gray-500 mb-3 transition-all duration-500 group-hover:text-gray-700">
          Address
    ...
    { !Object.keys(aUserTransactions).length ? (
      <div>Network issues please connect to a good network or refresh the browser to continue</div>
    ) : (
      <>
      {aUserTransactions && aUserTransactions.order && aUserTransactions.order.productDetails && aUserTransactions.order.productDetails.map((product: any) => (
        ...
        <div className="flex flex-col min-[500px]:flex-row min-[500px]:items-center justify-between">
          <div className="">
            ...
            <p className="font-normal text-xl leading-8 text-gray-500">
              Quantity : <span className="text-black font-semibold">{product && product.quantity}</span>
            </p>
          </div>
    ...
    <div className="data ">
      <p className="font-normal text-lg leading-8 text-gray-500 mb-11">
        you'll receive a shipping confirmation email when the items shipped
        successfully.
      </p>
